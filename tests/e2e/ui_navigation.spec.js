...
test.describe('UI and Navigation', () => {
  test('navigates to the Rules page from homepage', async ({ page }) => {
    ...
    const rulesLink = page
      .locator('a[href="/rules"], button:has-text("Rules"), [data-testid="rules-link"]')
      .first()
    ...
    const rulesPageHeading = page.locator(
      'h1:has-text("Rules of Koridor"), h2:has-text("Rules of Koridor"), [data-testid="rules-title"]'
    )
  ...
  test('switches between light and dark themes', async ({ page }) => {
    ...
    const themeSwitcherButton = page.locator(
      '[data-testid="theme-switcher"], .theme-toggle-button, #theme-button'
    ) // Adjust selector
