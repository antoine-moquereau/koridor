...
test.describe('Core Gameplay Mechanics (2-Player Local Game)', () => {
  ...
  test('attempt to place an invalid intersecting wall', async ({ page }) => {
    ...
    if ((await player1WallCountDisplay.count()) > 0) {
      await expect(player1WallCountDisplay).toHaveText(
        String(initialWallCountPlayer1AfterFirstWall)
      )
    }
    ...
    await expect(page.locator(`${intersectingWallLocation}.placed`)).not.toBeVisible({
      timeout: 1000
    })
    ...
    if ((await errorMessage.count()) > 0 && (await errorMessage.isVisible())) {
      ...
      console.log(
        'No visible error message for invalid wall placement, relying on other assertions.'
      )
    }
