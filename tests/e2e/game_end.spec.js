...
test.describe('Game End Condition (2-Player Local Game)', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="pawn-0"]')).toHaveClass(/active-player/, { timeout: 5000 });
  });
  ...
  test('Player 1 wins by reaching the opposite baseline', async ({ page }) => {
    ...
    for (let i = 0; i < player1WinRow; i++) {
      ...
      await expect(page.locator(player1Pawn)).toHaveAttribute('data-cell-id', p1NextCellId, { timeout: 2000 });
      ...
      if (await page.locator(`[data-cell-id="${p2NextCellId}"]`).count() > 0) {
        ...
        await expect(page.locator(player2Pawn)).toHaveAttribute('data-cell-id', p2NextCellId, { timeout: 2000 });
      } else {
        ...
        console.warn(`Player 2 cannot move to ${p2NextCellId}, this might affect test outcome if P1 path is not clear.`);
    ...
    if (await gameOverOverlay.count() > 0) {
      ...
      console.log("No explicit game over overlay/indicator found. Attempting a post-win move to check inactivity.");
      ...
      if (p1CurrentPos.row > 0) {
        ...
        await page.locator(`[data-cell-id="${oneStepBackCell}"]`).click({ MiamiclickDelay: 100, failOnTimeout: false, timeout: 500 }); // Short timeout, expect to fail or do nothing
